function map_clicked(e) {
	...
	marked_click = L.marker(e.latlng, {
		icon: new OurIcon({
			iconUrl: "/static/res/marker-icon-green.png"
		})
	});
...
function map_populate() {
	let map_type = $("#form-map-type").val()
	$.get("/map/retrieve/"+map_type, data => {
		for(let i = 0; i < data.length; i += 1) {
			L.marker([data[i].lat, data[i].lng]).addTo(mymap).bindPopup(gen_popup_desc(data[i]));
		}
	});
}

function gen_popup_desc(obj) {
	let c = "<b><pre>";
	c += "Title: " + obj.title + "\n";
	c += "Category: " + obj.category + "\n</pre></b>";
	c += "Address: " + obj.address + "<br><br>";
	c += "Description: <br>" + obj.desc;
	return c;
}
...
function clear_map() {
	for(l in mymap._layers)
		if(l != 26)
			mymap.removeLayer(mymap._layers[l]);
}

function populate_n_filter({ address = "", category = "", title = "" }) {
	let map_type = $("#form-map-type").val()
	$.get("/map/retrieve/"+map_type, data => {
		for(let i = 0; i < data.length; i += 1) {
			if(data[i].address.indexOf(address) == -1) continue;
			if(data[i].category.indexOf(category) == -1) continue;
			if(data[i].title.indexOf(title) == -1) continue;
			L.marker([data[i].lat, data[i].lng]).addTo(mymap).bindPopup(gen_popup_desc(data[i]));
		}
	});
}
...
$(() => {
	...
	mymap.on('locationfound', e => {
		L.marker(e.latlng, {
			icon: new OurIcon({
				iconUrl: "/static/res/marker-icon-red.png"
			})
